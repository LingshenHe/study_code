...
class block(nn.Module):
    ...
    def __init__(self, eqnum):
        ...
        self.conv1 =nn.Conv2d(6 * eqnum, block.c1 * eqnum, 1, stride=1, padding=0, groups=1)
        self.conv2 = nn.Conv2d(block.c1 * eqnum, block.c2*eqnum, 1, stride=1, padding=0)
        self.conv3=nn.Conv2d(block.c2*eqnum,eqnum,1,stride=1,padding=0)
        self.bn1 = nn.BatchNorm2d(block.c1 * eqnum)
        self.bn2 = nn.BatchNorm2d(block.c2*eqnum)
        self.bn3 = nn.BatchNorm2d(eqnum)
    ...
    def forward(self, x):
        ...
        y =F.selu(self.bn3(self.conv3(y)))
        return x + y
...
pde.steps=6
pde.eqnum=32
print('c1={},c2={},step={},eqnum={}'.format(block.c1,block.c2,pde.steps,pde.eqnum))
epoch = 350
batchsize = 128
...
for times in range(epoch):
    ...
    if sum_test>op:
        op=sum_test
        torch.save(net.state_dict(), 'step{}_c1{}c2{}_eqnum{}'.format(pde.steps,block.c1,block.c2,pde.eqnum))
