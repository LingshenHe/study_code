...
class block(nn.Module):
    ...
    def forward(self, x):
        ...
        y = F.softmin(self.bn2(self.conv2(y)))
        return x + y
...
net = pde().to(cuda)
linea = nn.Linear(16 * 16*pde.eqnum, 10).to(cuda)
...
for times in range(epoch):
    ...
    optimizer1 = torch.optim.SGD(linea.parameters(), weight_decay=decay_1, momentum=momentum_, lr=learning_rate)
    ...
    for x_train, y_train in train_loader:
        ...
        loss = nn.functional.cross_entropy(linea(net(x_train)), y_train)
        optimizer.zero_grad()
    ...
    if sum_test>op:
        op=sum_test
        torch.save(net.state_dict(), 'real_step{}_c{}_eqnum{}batchsize{}'.format(pde.steps,block.c,pde.eqnum,batchsize))
    print('real c={},step={},eqnum={},batchsize={}'.format(block.c, pde.steps, pde.eqnum, batchsize))
