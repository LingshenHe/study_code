...
net = pde().to(cuda)
net.load_state_dict(torch.load('00real_step{}_c{}_eqnum{}batchsize{}'.format(pde.steps,block.c,pde.eqnum,batchsize)))
momentum_ = 0.90
decay_ = 0.005
op=0.8
...
ss='np.savez('0001',conv=net.conv.weight.data.cpu().numpy()'
for i in range(step):
    ss=ss+',b'+str(i+1)+'=net.layer['+str(i)+'].conv1.weight.data.cpu().numpy()'
ss+=')'
exec(ss)
