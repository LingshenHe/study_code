...
class pde(nn.Module):
    ...
    def __init__(self):
        ...
        self.linear = nn.Linear(32 * 32*pde.eqnum*3, 10)
        ...
        self.layer=nn.ModuleList([block(3*pde.eqnum) for i in range(pde.steps)])


    def make(self, st):
        a = []
        for i in range(st):
            a.append(block(pde.eqnum*3))
        return nn.Sequential(*a)

    def forward(self, x):
        y=torch.zeros(x.size()[0],pde.eqnum*3,x.size()[2],x.size()[3]).to(cuda)
