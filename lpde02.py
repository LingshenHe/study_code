...
class block(nn.Module):
    ...
    def __init__(self, eqnum):
        ...
        self.conv1 =nn.Conv2d(5 * eqnum, eqnum, 1, stride=1, padding=0, groups=1)
        self.conv2 = nn.Conv2d(c * eqnum, eqnum, 1, stride=1, padding=0)
        self.bn1 = nn.BatchNorm2d(eqnum)
    ...
    def forward(self, x1):
        ...
        eqnum=pde.eqnum
        y=block.conv(x1)
        x=torch.zeros(x1.size(0),5*eqnum,x1.size(2),x1.size(3)).to(cuda)
        x[::,0:eqnum,::,::]=y[::,eqnum:2*eqnum,::,::]*y[::,eqnum:2*eqnum,::,::]+y[::,2*eqnum:3*eqnum,::,::]*\
            y[::,2*eqnum:3*eqnum,::,::]
...
class pde(nn.Module):
    ...
    def __init__(self):
        ...
        for i in range(6):
            block.conv.weight.data[i*pde.eqnum:(i+1)*pde.eqnum, 0, ::, ::] = b[i,::,::]
